...
- **IDs de ruta inválidos:** Si el cliente envía un `id` no numérico en `GET`, `PUT` o `DELETE /tasks/{id}`, Spring lanza `MethodArgumentTypeMismatchException`. `GlobalExceptionHandler` lo captura y responde con `{ "message": "Invalid task id: abc" }` (HTTP 400), manteniendo el contrato unificado de errores.
